...
- Phase: **Sprint hygiene + perf backlog** — **`TODOS.md` Open** (**0** cards). Shipped: **`getOpsSummary`** + cache, **bootstrap tables** in Drizzle, **client route code-split**, **equipment responsiva** (`.docx` + `**script/verify-responsiva-container.sh**`); **2026-04-09 P4 backlog umbrella** closed (residual niceties deferred) — see **`TODOS.md` Completed** / **`CHANGELOG.md`**.
- Last completed (2026-04-17): **`getOpsSummary`** — merged **`ops_events`** queries + **`server/ops-summary-cache.ts`**; tests **`server/tests/ops-summary-cache.test.ts`**, **`server/tests/ops-summary-parity.test.ts`**. Same period: **bootstrap tables** reconciled — Drizzle `user_sessions` / `login_rate_limits` / `ops_events`, **`migrations/add-bootstrap-tables.sql`**, **`npm run db:migrate:bootstrap-tables`**, **`server/tests/bootstrap-tables-parity.test.ts`** (**`CHANGELOG.md`**). Same period: **client bundle** — **`client/src/App.tsx`** lazy routes + **`Suspense`**; **`vite.config.ts`** **`manualChunks`**. Prior same sprint: **`express.json` / `urlencoded`** limits + **`413`** JSON + **`rawBody`** JSDoc (**`server/index.ts`**, **`server/tests/json-body-limit.test.ts`**); async **`fs`** upload paths; bulk inventory batched **`UPDATE`** + transactional history; aggregates SQL; **`GET /health`**; LIKE escape; delete consolidation; typed auth; Responsiva hardening — **`TODOS.md`** / **`CHANGELOG.md`**.
- Next up: **`TODOS.md` Open** empty — new work from **`CHANGELOG`** / product; or commit/review branch. **`npm run check`** (`tsc`) green; run **`npm test`** with Postgres for DB-heavy suites.
...
- 2026-04-17: `**getAuthUserId` + `requireAuthUser`** — centralize Passport user id extraction and 401 `**Sesión expirada`** for invalid session user after `requireAuth`; routes use helpers instead of `**(req as any).user`** — keeps `tsc` strict and aligns with `**Express.User`** in `**server/auth.ts**`.
- 2026-04-17: **JSON body cap + `rawBody` contract** — `**express.json` / `express.urlencoded`** use explicit `**1mb`** limit; `**req.rawBody`** populated in `**verify`** only for future inbound webhook signature use (documented; no consumer yet); oversized JSON returns `**413**` JSON before generic `**5xx**` error path.
- 2026-04-17: **Bootstrap tables in Drizzle + idempotent migration** — `user_sessions`, `login_rate_limits`, `ops_events` in `shared/schema.ts`; `migrations/add-bootstrap-tables.sql` + `**npm run db:migrate:bootstrap-tables**`; parity test `server/tests/bootstrap-tables-parity.test.ts`; startup ensures in auth / rate-limiter / ops-events unchanged; `docs/BACKUP-RESTORE.md` warns against `drizzle-kit push` dropping these tables.
...
- `**npm run db:push` safety:** Bootstrap tables (`user_sessions`, `login_rate_limits`, `ops_events`) are now in `shared/schema.ts` with operator migration `migrations/add-bootstrap-tables.sql` (`**npm run db:migrate:bootstrap-tables**`). If `push` still proposes **dropping** any of them, **abort** — indicates Drizzle/DB drift; fix definitions or apply the migration, do not confirm drop (`docs/BACKUP-RESTORE.md`).
...
Stopped at: **save state** — working tree on branch `**feat/que`** with many uncommitted changes (server routes, `**storage.ts`**, `**index.ts`**, tests, `**TODOS.md**`, `**CHANGELOG.md**`, Responsiva tests, etc.); see `**git status**`.

Next session should start with: new **`TODOS`** items as needed, or commit/review the current branch as appropriate; run `**npm test`** with Postgres when validating integration tests.